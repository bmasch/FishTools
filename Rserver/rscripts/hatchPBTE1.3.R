...
y=as.numeric(unlist(strsplit(GET$y,"\n")))
z=as.numeric(unlist(strsplit(GET$z,"\n")))
Nsamp=as.numeric(GET$Nsamp)
x1=as.numeric(GET$x1)
n1=as.numeric(GET$n1)
n=as.numeric(GET$n)
lambda=as.numeric(unlist(strsplit(GET$lambda,"\n")))
ppbt=as.numeric(unlist(strsplit(GET$ppbt,"\n")))
type=GET$type
BOOT=FALSE
if(type=="B")BOOT=TRUE
NBOOT=as.numeric(GET$NBOOT)
...
doCalcTest = function(){
   tmp=list(Nsamp=Nsamp,x1=x1,n=n,n1=n1,y=y,z=z,lambda=lambda,ppbt=ppbt,BOOT=FALSE,NBOOT=NBOOT)
   cat(toJSON(tmp))
}

checkLength=function(dat){
  if(length(dat)==1) return(dat[1])
  else return(dat)
}
...
doCalc= function(){
  ...
  if(type=="A"){
	...
	dat <- phos.pbte.main(Nsamp=Nsamp,x1=x1,n=n,n1=n1,y=y,z=z,lambda=lambda,ppbt=ppbt,BOOT=FALSE,NBOOT=NBOOT)
	#dat <- phos.pbt.estimates()
	
	tmp <- list(Nsamp=dat$Nsamp,
	x1=dat$x1,
	n=dat$n1+dat$n2,
	n1=dat$n1,
	y=dat$y,
	...
	dat <- phos.pbte.main(Nsamp=Nsamp,x1=x1,n=n,n1=n1,y=y,z=z,lambda=lambda,ppbt=ppbt,BOOT=TRUE,NBOOT=NBOOT)
	tmp <- list(Nsamp=dat$Nsamp,
	x1=dat$x1,
	n=dat$n1+dat$n2,
	n1=dat$n1,
