...
Nhos=as.numeric(GET$Nhos)
lambda=as.numeric(GET$lambda)
...
doCalc= function(){
  ...
  if(type=="A"){
	...
	dat <- main(MONTE=FALSE,Nsims=NA,Nhos=Nhos,Nnos=Nnos,theta=theta,lambda=lambda)
	...
	dat <- main(MONTE=TRUE,Nsims=NSIM,Nhos=Nhos,Nnos=Nnos,theta=theta,lambda=lambda)
	tmp <- list(Nnos=dat$Nnos,
	Nhos=dat$Nhos,
	theta=dat$theta,
	lambda=dat$lambda,
