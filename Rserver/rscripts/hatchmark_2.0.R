...
Nnos=as.numeric(GET$Nnos)
Nhos=as.numeric(unlist(strsplit(GET$Nhos,"\n")))
theta=as.numeric(unlist(strsplit(GET$theta,"\n")))
lambda=as.numeric(unlist(strsplit(GET$lambda,"\n")))
phi=as.numeric(unlist(strsplit(GET$phi,"\n")))
type=GET$type
...
doCalc= function(){
  ...
  if(type=="A"){
	...
	dat <- phos.mhatch.estimates2(Nhos=Nhos,Nnos=Nnos,theta=theta,lambda=lambda,pcwt=phi)
	tmp <- list(Nnos=dat$Nnos,
	Nhos=dat$Nhos,
	theta=dat$theta,
	lambda=dat$lambda,
	phi=dat$pcwt,
	...
	NSIM=NA,
	phos=dat$phos,
	SE_Nnos=dat$SE2.Nnoshat,
	CV_Nnos=dat$CV2.Nnoshat,
	SE_Nhos=dat$SE2.Nhoshat,
	CV_Nhos=dat$CV2.Nhoshat,		  
	SE_phos=dat$SE2.phoshat,
	CV_phos=dat$CV2.phoshat,
	BIAS_phos=NA,
	warning = warn)
	...
	dat <- phos.mhatch.estimates1(Nsims=NSIM,Nhos=Nhos,Nnos=Nnos,theta=theta,lambda=lambda,pcwt=phi)
	tmp <- list(Nnos=dat$Nnos,
	Nhos=dat$Nhos,
	theta=dat$theta,
	lambda=dat$lambda,
	...
	NSIM=NSIM,
	phos=dat$phos,
	SE_Nnos=dat$SE.Nnoshat,
	CV_Nnos=dat$CV.Nnoshat,
	SE_Nhos=dat$SE.Nhoshat,
	CV_Nhos=dat$CV.Nhoshat,		  
	SE_phos=dat$SE.phoshat,
	CV_phos=dat$CV.phoshat,
	BIAS_phos=dat$BIAS.phoshat,
	warning = warn)
	time=proc.time()[3]-time1
	doLog(toJSON(GET),time,'TRUE',warn)	
	cat(toJSON(tmp))  
