...
phosi=as.numeric(unlist(strsplit(GET$phosi,"\n")))
...
doCalc= function(){
  ...
  if(type=="A"){
	...
	dat <- phos.pbt.main(phosi=phosi,Nsamp=Nsamp,n=n,n1=n1,lambda=lambda,ppbt=ppbt,OPT=OPT,MONTE=FALSE)
	#dat <- phos.pbt.estimates()
	
	tmp <- list(phosi=dat$phosi,
	Nsamp=dat$Nsamp,
	n=dat$n,
	n1=dat$n1,
	...
	Ex1=dat$Ex1,
	Ex2=dat$Ex2,
	SE_MIN=dat$SE_MIN.phos,
	CV_MIN=dat$CV_MIN.phos,
	SE_phos=dat$SE.phos,
	CV_phos=dat$CV.phos,
	Bias_phos=NA,
	warning = warn)
	...
	dat <- phos.pbt.main(phosi=phosi,Nsamp=Nsamp,n=n,n1=n1,lambda=lambda,ppbt=ppbt,OPT=OPT,MONTE=TRUE,NSIM=NSIM)
	tmp <- list(phosi=dat$phosi,
	Nsamp=dat$Nsamp,
	n=dat$n,
	n1=dat$n1,
